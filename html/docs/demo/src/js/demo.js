(function($) {
  ...
  function badgesWithScrollAnimationExample() {
    ...
    $badges.waypoint({
      handler: function(direction) {
        ...
        var removeCssClasses = function() {
          $element.removeClass(cssClasses.join(' '));
        };

        $.support.animation ?
          $element.one('bsAnimationEnd', removeCssClasses) :
          removeCssClasses();
      },
  ...
  function sliderWithFashionModelExample() {
    ...
    $fashionModelSlider.on('update', function(evt, values, handle) {
      if ($fashionModelSlider.get() < 170) {
        ...
        toastr.warning('The minimum height in order to become a model in our ' +
          'agency is currently 170 cm in height (without heels).');
      }
  ...
  function bootstrapTableCustomCheckboxFormatter(value, row, index) {
    return '<label class="custom-control custom-control-primary custom-checkbox">' +
      '<input class="custom-control-input" type="checkbox" name="btSelectItem" data-index="' + index + '">' +
  ...
  function bootstrapTableSwitchFormatter(value, row, index) {
    return '<label class="switch switch-primary">' +
      '<input class="switch-input" type="checkbox" name="btSelectItem" data-index="' + index + '">' +
      ...
      $datatables.DataTable({
        ...
        initComplete: function() {
          this.api().columns().every(function() {
            ...
            var select = $('<select class="demo-select2"><option value="">' + $(header).text() + '</option></select>')
  ...
  function datatablesButtonsExample() {
    ...
    $datatablesButtons.buttons().container()
      .appendTo('#demo-datatables-buttons-1_wrapper .col-sm-6:eq(0)');
  }
  ...
  function datatablesButtonsTableBorderedExample() {
    ...
    $datatablesButtons.buttons().container()
      .appendTo('#demo-datatables-buttons-2_wrapper .col-sm-6:eq(0)');
  }
    ...
    if ($datatablesFixedheader.length) {
      ...
      var adjustFixedHeader = function() {
        ...
        adjustFixedHeaderTimeoutId = setTimeout(function() {
          $datatablesFixedheader.columns.adjust()
  ...
  function datatablesResponsiveTableBorderedExample() {
    ...
    $datatablesResponsive.DataTable({
      responsive: true,
      dom: "<'row'<'col-sm-6 col-sm-push-6'i><'col-sm-6 col-md-pull-6'f>>" +
        "<'row'<'col-sm-12'tr>>" +
        "<'row'<'col-sm-6'l><'col-sm-6'p>>",
      language: {
        paginate: {
  ...
  function datatablesRowreorderTableBorderedExample() {
    ...
    $datatablesRowreorder.buttons().container()
      .appendTo('#demo-datatables-rowreorder-2_wrapper .col-sm-6:eq(0)');
  }
    ...
    if ($openStreetMap.length) {
      ...
      openStreetMap.addMapType('osm', {
        getTileUrl: function(coord, zoom) {
          return 'https://a.tile.openstreetmap.org/' + zoom +
            '/' + coord.x + '/' + coord.y + '.png';
        },
        maxZoom: 18,
        name: 'OpenStreetMap',
  ...
  function chartWithBubblesExample() {
    var $bubbleChart, bubbleChart, breakpoint, config, categories,
      randomSales, randomDifference, randomPercentage;
  ...
  function chartIssuesExample() {
    ...
    if ($issuesChart.length) {
      ...
      randomIssues = function() {
        return getRandomInt(1000, 2000);
      };

      randomRankings = function() {
  ...
  function showShareMessage() {
    ...
    message = 'If you like Elephant, please share it with your friends ' +
      'and followers, this way you will help the elephant grow.';
  ...
  // Share message
  $('button.theme-panel-toggler')
  	.one('click', showShareMessage);
