(function($) {
  ...
  var App = {
    ...
    bindEvents: function() {
      ...
      this.$sidenav.on('collapse-start.e.app', this.handleSidenavCollapseStart.bind(this))
        .on('expand-start.e.app', this.handleSidenavExpandStart.bind(this));

      this.$sidenav.on('collapse-end.e.app', this.handleSidebarStickyUpdate.bind(this))
        .on('expand-end.e.app', this.handleSidebarStickyUpdate.bind(this));

      this.$sidenav.on('shown.metisMenu.e.app', this.handleSidebarStickyUpdate.bind(this))
        .on('hidden.metisMenu.e.app', this.handleSidebarStickyUpdate.bind(this));
    ...
    handleSidenavToggle: function(evt) {
      evt.preventDefault();
      this.toggleSidenav();
    },
    handleSidenavCollapseStart: function(evt) {
      var $input = this.getThemeSettingsBy(
        this.CssClasses.LAYOUT_SIDEBAR_COLLAPSED
      );

      $input.prop('checked', true);
    },
    handleSidenavExpandStart: function(evt) {
      var $input = this.getThemeSettingsBy(
        this.CssClasses.LAYOUT_SIDEBAR_COLLAPSED
      );
    ...
    handleSidebarStickyUpdate: function(evt) {
      if (this.isSidebarSticky()) {
        this.updateStickySidebar();
      }
    },
    handleSearchFormToggle: function(evt) {
      evt.preventDefault();
      this.toggleSearchForm();
    },
    handleCardToggle: function(evt) {
      this.$card = $(evt.target).closest('.' + this.CssClasses.CARD);

      this.$card.toggleClass(this.CssClasses.CARD_COLLAPSED)
        .find('.' + this.CssClasses.CARD_BODY).slideToggle();

      if (this.$card.hasClass(this.CssClasses.CARD_COLLAPSED)) {
        this.$card.attr('aria-expanded', false);
        this.$cardTogglerBtn.attr('aria-expanded', false)
          .attr('title', 'Expand');
      } else {
        this.$card.attr('aria-expanded', true);
        this.$cardTogglerBtn.attr('aria-expanded', true)
          .attr('title', 'Collapse');
      }

      evt.preventDefault();
    },
    handleCardReload: function(evt) {
    ...
    handleCardFocus: function(evt) {
      this.$body.toggleClass(this.CssClasses.CARD_FOCUS_MODE);

      $(evt.target).closest('.' + this.CssClasses.CARD)
        .toggleClass(this.CssClasses.CARD_FOCUSED);

      evt.preventDefault();
    },
    handleCardRemove: function(evt) {
    ...
    handleThemePanelToggle: function(evt) {
      evt.preventDefault();
      this.toggleThemePanel();
    },
    handleThemeSettingsChange: function(evt) {
    ...
    collapseSidenav: function() {
      ...
      this.transitionTimeoutId = setTimeout(function() {
        this.$sidenav.animate({opacity: 1})
    ...
    toggleSearchForm: function() {
      ...
      if (this.isSearchFormCollapsed()) {
        ...
        this.$searchFormBtn.attr('aria-expanded', false)
          .attr('title', 'Expand search form ( S )');
      } else {
        this.$searchForm.attr('aria-expanded', true);
        this.$searchFormBtn.attr('aria-expanded', true)
          .attr('title', 'Collapse search form ( S )');
      }
    },
    isSearchFormCollapsed: function() {
      return this.$searchForm.hasClass(
        this.CssClasses.SEARCH_FORM_COLLAPSED
      );
    },
    toggleThemePanel: function() {
      ...
      if (this.isThemePanelCollapsed()) {
        ...
        this.$themePanelBtn.attr('aria-expanded', false)
          .attr('title', 'Expand theme panel ( ] )');
      } else {
        this.$themePanel.attr('aria-expanded', true);
        this.$themePanelBtn.attr('aria-expanded', true)
          .attr('title', 'Collapse theme panel ( ] )');
      }
    },
    isThemePanelCollapsed: function() {
      return this.$themePanel.hasClass(
        this.CssClasses.THEME_PANEL_COLLAPSED
      );
    },
    ...
    changeThemeSettings: function(settings) {
    ...
    getThemeSettingsBy: function(name) {
      return this.$themeSettings.filter("[name='" + name + "']");
    },
    isHeaderStatic: function() {
      return !this.$layout.hasClass(
        this.CssClasses.LAYOUT_HEADER_FIXED
      );
    },
    setHeaderFixed: function(state) {
      var settings = {};
    ...
    isSidebarFixed: function() {
      return this.$layout.hasClass(
        this.CssClasses.LAYOUT_SIDEBAR_FIXED
      );
    },
    setSidebarFixed: function(state) {
      var settings = {},
      ...
      this.changeThemeSettings(settings)
        .addCustomScrollbarTo($sidebar);
    },
    isSidebarSticky: function() {
      return this.$layout.hasClass(
        this.CssClasses.LAYOUT_SIDEBAR_STICKY
      );
    },
    setSidebarSticky: function(state) {
      var settings = {};
    ...
    destroyStickySidebar: function() {
      ...
      $target.data('hcSticky') && $target.hcSticky('destroy')
    ...
    getCreateOptions: function(prefix) {
    ...
    getBlockUIOptions: function() {
      ...
      $spinner.addClass(this.CssClasses.SPINNER)
        .addClass(this.CssClasses.SPINNER_PRIMARY);
